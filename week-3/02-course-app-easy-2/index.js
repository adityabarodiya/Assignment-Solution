...
app.post("/admin/signup", (req, res) => {
  ...
  if (existingAdmin) res.status(403).json({ massage: "Admin already exits" });
  else {
    ADMINS.push(admin);
...
app.post("/admin/login", (req, res) => {
  ...
  if (admin) {
    const token = generateJwt(admin);
    res.json({ masssge: "Admin login succesfully", token });
...
app.post("/users/signup", (req, res) => {
  ...
  if (existingUser) {
    ...
    res.json({ massage: "User Created succecfully", token });
  }
});
...
app.post("/users/login", (req, res) => {
  ...
  if (user) {
    ...
    res.status(403).json({ massage: "User Authentication failed" });
  }
});
